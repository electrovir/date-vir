...
import {DurationUnit, type AnyDuration, type Duration} from '@date-vir/duration';
import {ConversionAccuracy} from 'luxon';
import {roundDuration} from '../extra-utils/round-duration.js';
...
> {
    ...
    if (diffType === DiffType.AdditiveUnits) {
        ...
        throw new Error(
            `Failed to diff dates: provided diffType is invalid: '${String(diffType)}'`,
        );
    }
}
